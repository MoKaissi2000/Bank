...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['sender_acct_no'], $_POST['receiver_acct_no'], $_POST['amount'])) {
        ...
        $sql_sender = "UPDATE Investment SET Balance = Balance - ? WHERE Acct_no = ?";
        $stmt_sender = $conn->prepare($sql_sender);
        $stmt_sender->bind_param("di", $amount, $sender_acct_no);
        $stmt_sender->execute();

        // Update receiver's balance
        $sql_receiver = "UPDATE Investment SET Balance = Balance + ? WHERE Acct_no = ?";
        ...
        if ($stmt_sender->affected_rows > 0 && $stmt_receiver->affected_rows > 0) {
            ...
            $sql_trans_sender = "INSERT INTO investment_transactions (trans_type, trans_date, trans_amount, lastname, firstname, phone) SELECT ?, ?, ?, lastname, firstname, phone FROM Investment WHERE Acct_no = ?";
            ...
            $sql_trans_receiver = "INSERT INTO investment_transactions (trans_type, trans_date, trans_amount, lastname, firstname, phone) SELECT ?, ?, ?, lastname, firstname, phone FROM Investment WHERE Acct_no = ?";
